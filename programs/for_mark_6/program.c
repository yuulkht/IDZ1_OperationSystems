...
int main(int argc, char *argv[]) {
    ...
    int pipefd[2];
    if (pipe(pipefd) == -1) {
        perror("Ошибка при создании неименованного канала");
        exit(EXIT_FAILURE);
    }
    ...
    // Процесс 1: Чтение из канала и запись в выходной файл
    ...
    bytes_read_result = read(pipefd[0], buffer_result, BUFFER_SIZE);
